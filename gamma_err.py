...
def remove_error(data, param, bins=25, pthresh=.5, verbose=0):
    ...
    if verbose > 0:     # histogram and fitting
        ...
        x = np.linspace(0, stats.gamma.ppf(0.99, alpha), 1000)
        ...
        plt.axvline(stats.gamma.ppf(pthresh, alpha), color='navy', linestyle='-.', label='Max Threshold')
        plt.legend(loc='best')
        plt.xlabel(param)
        plt.ylabel('counts')
        plt.show()
    ...
    data = data[data[param] <= stats.gamma.ppf(pthresh, alpha)]
    return data
